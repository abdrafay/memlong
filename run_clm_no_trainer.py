...
def main():
    ...
    if not args.continual_finetuning:
        ...
        llama_config.mem_layer = 0
        llama_config.ret_attn_layers = args.ret_attn_layers
        llama_config.ret_attn_layers = [0, 1, 2] if llama_config.ret_attn_layers is None else llama_config.ret_attn_layers
        llama_config.memory_size = 512 if args.memory_size is None else args.memory_size 
        ...
        # peft priority
        if args.peft_model:
            llama_config = LlamaConfig.from_pretrained(r"D:\Rafay\Freelance\MemLong-main\config.json")

            # Check and set ret_attn_layers if None
        if llama_config.ret_attn_layers is None:
            print("ret_attn_layers is None, setting default value to [0, 1, 2].")
            llama_config.ret_attn_layers = [0, 1, 2]  # or whatever default you want
