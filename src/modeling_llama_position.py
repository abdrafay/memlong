...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        ret_attn_layers: List[int] = field(default_factory=list)
        assert isinstance(config.ret_attn_layers, list), f"Expected a list for ret_attn_layers, got {type(config.ret_attn_layers)} with value {config.ret_attn_layers}"
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def __init__(self, config, toolkit_config:ToolkitConfig):
        ...
        print(config.max_position_embeddings, self.toolkit.get_max_seq_length(),config.mem_group_size, "CHECKING EMBEDDING LENGTH")
        self.context_window_length = min(config.max_position_embeddings, self.toolkit.get_max_seq_length(),512)
